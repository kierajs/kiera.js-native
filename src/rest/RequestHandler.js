...
const HTTPS = require("@kierajs/react-native-http");
...
class RequestHandler {
    constructor(client, forceQueueing) {
        ...
        this.userAgent = `HelseliaBot (https://github.com/kierajs/kiera.js, ${require("../../package.json").version})`;
        this.ratelimits = {};
    ...
    */
                ...
                req.once("response", (resp) => {
                    ...
                    if(this.latencyRef.lastTimeOffsetCheck < Date.now() - 5000) {
                        ...
                        if(this.latencyRef.timeOffset - this.latencyRef.latency >= this._client.options.latencyThreshold && timeOffset - this.latencyRef.latency >= this._client.options.latencyThreshold) {
                            this._client.emit("warn", new Error(`Your clock is ${this.latencyRef.timeOffset}ms behind Helselia's server clock. Please check your connection and system time.`));
                        }
                    ...
                    _respStream.on("data", (str) => {
                        ...
                        // Helselia breaks RFC here, using milliseconds instead of seconds (╯°□°）╯︵ ┻━┻
                        // This is the unofficial Helselia dev-recommended way of detecting that
                        ...
                        if(resp.statusCode >= 300) {
                            ...
                            let err;
                            if(response.code) {
                                err = new HelseliaRESTError(req, resp, response, stack);
                            } else {
                                err = new HelseliaHTTPError(req, resp, response, stack);
                            }
                            reject(err);
                            return;
                        }
    ...
    routefy(url, method) {
        let route = url.replace(/\/([a-z-]+)\/(?:[0-9]{17,19})/g, function(match, p) {
            return p === "channels" || p === "clubs" || p === "webhooks" ? match : `/${p}/:id`;
