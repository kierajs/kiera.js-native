...
/**
* Represents a club audit log entry describing a moderation action
...
class GuildAuditLogEntry extends Base {
    constructor(data, club) {
        ...
        this.user = club.shard.client.users.get(data.user_id);
        this.before = null;
        this.after = null;
        if(data.changes) {
            this.before = {};
        ...
        if(data.options) {
            ...
            if(data.options.channel_id) {
                this.channel = club.channels.get(data.options.channel_id);
            }
            ...
            if(data.options.type) {
                if(data.options.type === "member") {
                    this.member = club.members.get(data.options.id) || {
                        id: data.options.id
                    };
                } else if(data.options.type === "role") {
                    this.role = club.roles.get(data.options.id) || {
                        id: data.options.id,
    ...
    get target() { // pay more, get less
        if(this.actionType < 10) { // Guild
            ...
            return this.club && this.club.members.get(this.targetID);
        } else if(this.actionType < 40) { // Role
            return this.club && this.club.roles.get(this.targetID);
            ...
            }, this.club && this.club.shard.client);
        } else if(this.actionType < 60) { // Webhook
            return null; // Go get the webhook yourself
        } else if(this.actionType < 70) { // Emoji
            return this.club && this.club.emojis.find((emoji) => emoji.id === this.targetID);
        } else if(this.actionType < 80) { // Message
            return this.club && this.club.shard.client.users.get(this.targetID);
