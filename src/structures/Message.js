...
* @prop {String} [clubID] The ID of the club this message is in (undefined if in DMs)
* @prop {String} id The ID of the message
* @prop {String} jumpLink The url used by Helselia clients to jump to this message
...
* @prop {String} messageReference.clubID The id of the club this message was crossposted from
...
class Message extends Base {
    constructor(data, client) {
        ...
        if(data.message_reference) {
            this.messageReference = {
                ...
                clubID: data.message_reference.club_id
            };
        } else {
        ...
        if(this.channel.club) {
            if(data.member) {
                data.member.id = this.author.id;
                this.member = this.channel.club.members.update(data.member, this.channel.club);
            } else if(this.channel.club.members.has(this.author.id)) {
                this.member = this.channel.club.members.get(this.author.id);
            } else {
                this.member = null;
            }

            if(!this.clubID) {
                this.clubID = this.channel.club.id;
            }
        } else {
            this.member = null;
        }
        ...
        switch(this.type) {
            ...
            case MessageTypes.CLUB_MEMBER_JOIN: {
                data.content = SystemJoinMessages[~~(this.createdAt % SystemJoinMessages.length)].replace(/%user%/g, this.author.mention);
                break;
            }
            case MessageTypes.USER_PREMIUM_CLUB_SUBSCRIPTION: {
                data.content = `${this.author.mention} just boosted the server!`;
                break;
            }
            case MessageTypes.USER_PREMIUM_CLUB_SUBSCRIPTION_TIER_1:
            case MessageTypes.USER_PREMIUM_CLUB_SUBSCRIPTION_TIER_2:
            case MessageTypes.USER_PREMIUM_CLUB_SUBSCRIPTION_TIER_3: {
                data.content = `${this.author.mention} just boosted the server! ${this.channel.club ? this.channel.club.name : data.club_id} has achieved **Level ${this.type - 8}!**`;
                break;
            }
            ...
            case MessageTypes.CLUB_DISCOVERY_DISQUALIFIED: {
            ...
            case MessageTypes.CLUB_DISCOVERY_REQUALIFIED: {
                data.content = "This server is eligible for Server Discovery again and has been automatically relisted!";
                break;
            }
            case MessageTypes.CLUB_DISCOVERY_GRACE_PERIOD_INITIAL_WARNING: {
            ...
            case MessageTypes.CLUB_DISCOVERY_GRACE_PERIOD_FINAL_WARNING: {
    ...
    update(data, client) {
        if(data.content !== undefined) {
            ...
            this.mentions = data.mentions.map((mention) => {
                ...
                if(mention.member && this.channel.club) {
                    mention.member.id = mention.id;
                    this.channel.club.members.update(mention.member, this.channel.club);
                }
                return user;
            });
    ...
    get cleanContent() {
        ...
        if(this.channel.club) {
            const member = this.channel.club.members.get(this.author.id);
        ...
        if(this.mentions) {
            this.mentions.forEach((mention) => {
                if(this.channel.club) {
                    const member = this.channel.club.members.get(mention.id);
                    if(member && member.nick) {
        ...
        if(this.channel.club && this.roleMentions) {
            for(const roleID of this.roleMentions) {
                const role = this.channel.club.roles.get(roleID);
    ...
    get jumpLink() {
        return `${Endpoints.CLIENT_URL}${Endpoints.MESSAGE_LINK(this.channel.type === 1 ? "@me" : this.channel.club.id, this.channel.id, this.id)}`;
    }
